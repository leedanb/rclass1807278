...
#class_scores.csv파일을 csv파일로 불러온 것
read.csv("class_scores.csv")
#sores라는 변수에 class_scores.csv파일 읽은 것을 데이터프레임으로 넣은 것
...
#######################################################
## 문제 4. 1학년 학생들을 학번, 등급, 성별을 출력 하시오. 단, 학번 내림차순 정렬
#######################################################
...
## !grade == 1
## top_n(n=3,wt=수학)
## n은 상위 3개, wt 는 내림차순 기준
#######################################################

scores %>% 
  dplyr::filter(!학년 == 1) %>% 
  dplyr::top_n(n=3, wt=수학) %>% 
  dplyr::arrange(desc(수학))
